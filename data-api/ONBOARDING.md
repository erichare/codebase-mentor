...
| **Task / DBTask** | The table-path equivalent of an operation step. `BaseTask` manages state transitions (`UNINITIALIZED → READY → IN_PROGRESS → COMPLETED/ERROR/SKIPPED`) and retry loops via `TaskRetryPolicy`. |
